...
class Entity implements EntityInterface
{
     ...
     * @return void
     */
    public function generateFeature($model, $project, $definition, $parent_model): void
    {
        if (SchemaHierarchy::class !== $definition['namespace']) {
            throw new \Exception("definition not recognized", 1);
        }
        ...
        $schema_hierarchy = (app()->make($definition['namespace']))->get();
        $functionality_schema_namespace = str_replace('@', '', $schema_hierarchy['hierarchy']['trident-vista']['{{entity_name}}']['Features']['{{feature_name}}'][0]);
        $functionality_schema_definition = app()->make($functionality_schema_namespace);
        
        $request_schema_namespace = str_replace('@', '', $schema_hierarchy['hierarchy']['trident-vista']['{{entity_name}}']['Features']['{{feature_name}}'][1]);
        $request_schema_definition = app()->make($request_schema_namespace);
        
        $response_schema_namespace = str_replace('@', '', $schema_hierarchy['hierarchy']['trident-vista']['{{entity_name}}']['Features']['{{feature_name}}'][2]);
        $response_schema_definition = app()->make($response_schema_namespace);
        ...
        //write json files to project directory
        //make folder
        $path = $project['root_folder'].'/'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'];
        if (!is_dir(dirname($path.'/.'))) {
            mkdir(dirname($path.'/.'), 0777, true);
        }
        file_put_contents($path.'/Functionality.json', json_encode($functionality_json, JSON_PRETTY_PRINT));
        ...
        $command  = 'cd '.str_replace('\\', '/', $project['root_folder']).' && ';
        $command .= 'php ';
        $command .= 'artisan trident:generate:workflow_logic_function '.$parent_model['name'].' '.$model['name'].' ';
        $command .= '--functionality_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Functionality.json'.'" ';
        $command .= '--resource_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Response.json'.'" ';
        $command .= '--validation_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Request.json'.'" ';
        $command .= '--strict_type_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Request.json'.'" ';
     ...
     * @return void
     */
    public function refreshFeature($model, $project, $definition, $parent_model): void
    {
        if (SchemaHierarchy::class !== $definition['namespace']) {
            throw new \Exception("definition not recognized", 1);
        }
        ...
        $command  = 'cd '.str_replace('\\', '/', $project['root_folder']).' && ';
        $command .= 'php ';
        $command .= 'artisan trident:refresh:workflow_logic_function '.$parent_model['name'].' '.$model['name'].' ';
        $command .= '--functionality_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Functionality.json'.'" ';
        $command .= '--resource_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Response.json'.'" ';
        $command .= '--validation_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Request.json'.'" ';
        $command .= '--strict_type_schema_path="'.$project['relative_schemas_folder'].'/'.$parent_model['name'].'/Features/'.$model['name'].'/'.'Request.json'.'" ';
